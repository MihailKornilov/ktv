...
$send="<DIV id=repRow>";
$admins=$ktv->QueryRowArray("select distinct(admin_add) from abonentka where tip=0 and dtime_add LIKE '".$_GET['year']."-%'".$rule." order by admin_add");
if(count($admins)>0)
	{
...
for($n=1;$n<=12;$n++)
	{
	$abon=$ktv->QueryRowOne("select count(id),sum(money) from abonentka where tip=0 and dtime_add LIKE '".$_GET['year']."-".($n<10?0:'').$n."%'".$rule);
	$send.="<TR>";
...
$abon=$ktv->QueryRowOne("select count(id),sum(money) from abonentka where tip=0 and dtime_add LIKE '".$_GET['year']."-%'".$rule);
