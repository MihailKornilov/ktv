...
for($n=1;$n<=12;$n++)
	{
	$podkl=$ktv->QRow("select count(id) from abonent abonent where date_podkl LIKE '".$_GET['year']."-".($n<10?'0':'').$n."%' ".$rule);
	$send.="<TR align=right>";
	$send.="<TD>".($podkl>0?"<A HREF='/report/".$_GET['year']."/".$n."/Podkl'>".fullmonth($n)."</A>":fullmonth($n));
	$send.="<TD align=center><B>".($podkl>0?$podkl:'&nbsp;')."</B>";
	}
$podkl=$ktv->QRow("select count(id) from abonent abonent where date_podkl LIKE '".$_GET['year']."-%' ".$rule);
