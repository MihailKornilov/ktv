...
for($n=1;$n<=12;$n++)
	{
	$uslugi=$ktv->QueryRowOne("select count(id),sum(money) from abonentka where tip=1 and dtime_add LIKE '".$_GET['year']."-".($n<10?0:'').$n."%'".$rule."and admin_add in (".$_GET['admin'].")");
	$send.="<TR>";
...
$uslugi=$ktv->QueryRowOne("select count(id),sum(money) from abonentka where tip=1 and dtime_add LIKE '".$_GET['year']."-%'".$rule."and admin_add in (".$_GET['admin'].")");
