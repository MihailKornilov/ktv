...
for($n=1;$n<=date("t", strtotime($_GET['year']."-".($_GET['mon']<10?0:'').$_GET['mon']."-01"));$n++)
	{
	$podkl=$ktv->QRow("select count(id) from abonent abonent where date_podkl='".$_GET['year']."-".$_GET['mon']."-".$n."' ".$rule);
	$send.="<TR align=right>";
	$send.="<TD>".($podkl>0?"<A HREF='/report/".$_GET['year']."/".$_GET['mon']."/".$n."'>".$n."</A>":$n);
	$send.="<TD align=center><B>".($podkl>0?$podkl:'&nbsp;')."</B>";
	}
$podkl=$ktv->QRow("select count(id) from abonent abonent where date_podkl LIKE '".$_GET['year']."-".(abs($_GET['mon'])<10?'0':'').$_GET['mon']."-%' ".$rule);
