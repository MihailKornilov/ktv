...
$send="<DIV id=repRow>";
$admins=$ktv->QueryRowArray("select distinct(admin_add) from abonentka where tip=1 and dtime_add LIKE '".$_GET['year']."-".($_GET['mon']<10?0:'').$_GET['mon']."-%'".$rule." order by admin_add");
if(count($admins)>0)
	{
...
for($n=1;$n<=date("t", strtotime($_GET['year']."-".($_GET['mon']<10?0:'').$_GET['mon']."-01"));$n++)
	{
	$uslugi=$ktv->QueryRowOne("select count(id),sum(money) from abonentka where tip=1 and dtime_add LIKE '".$_GET['year']."-".($_GET['mon']<10?0:'').$_GET['mon']."-".($n<10?0:'').$n."%'".$rule);
	$send.="<TR align=right>";
	$send.="<TD>".($uslugi[0]>0?"<A HREF='/report/".$_GET['year']."/".$_GET['mon']."/".$n."'>".$n."</A>":$n);
	$send.="<TD align=center><B>".($uslugi[0]>0?$uslugi[0]:'&nbsp;')."</B>";
	$send.="<TD>".$uslugi[1];
	}
$uslugi=$ktv->QueryRowOne("select count(id),sum(money) from abonentka where tip=1 and dtime_add LIKE '".$_GET['year']."-".($_GET['mon']<10?0:'').$_GET['mon']."-%'".$rule);
